...
// Burger menu that aggregates all major feature buttons.
// ═══════════════════════════════════════════════════════════════
...
const NavMenu = (() => {
  ...
  // Map of menu item IDs → click actions
  const menuItems = [
    {
      id:    'nm-equalizer',
      label: 'EQUALIZER',
    ...
    {
      id:    'nm-smart-playlist',
      label: 'SMART PLAYLIST',
      icon:  `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"/>
              </svg>`,
      action: () => { if (window.SmartPlaylist) SmartPlaylist.open(); },
    },
    {
      id:    'nm-lyrics',
      label: 'LYRICS',
    ...
    },
    {
      ...
      action: () => { if (window.RTV) RTV.openRadio(); },
    },
    {
      id:    'nm-tv',
      label: 'TV ONLINE',
      ...
      action: () => { if (window.RTV) RTV.openTv(); },
    },
    {
      id:    'nm-convert',
      label: 'CONVERTER',
  ...
  function _buildMenu() {
    ...
    menuItems.forEach((item, idx) => {
      ...
      el.innerHTML = `
        ...
        <svg class="nm-item-arrow" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="9 18 15 12 9 6"/>
        </svg>
      `;
      el.addEventListener('click', () => {
        close();
        setTimeout(() => item.action(), 80); // slight delay so menu closes first
      });
      list.appendChild(el);
    });
  }
